...
import type { DsExtraInvoiceTax } from "@/types/donaive-software";
...
export function computeLineRealCosts(
  ...
  const extraTaxesInInvoiceCurrency = extraTaxes.map((t) => {
    ...
    const amount = currency === "BS" ? bs : rate > 0 ? bs / rate : bs;
  ...
  for (const l of lines) {
    ...
    const qtyReceived = Math.max(0.0001, lineQtyReceived(l));
    const realUnit = realTotal / qtyReceived;
    const upp = lineQtyBase(l) > 0 ? lineQtyBase(l) / Math.max(1, l.qty) : l.unitsPerBox;
    const realBox = realUnit * upp;
...
export function newExtraTax(name = ""): DsExtraInvoiceTax {
  return {
